...
Jako że jest to w pełni statyczna apka, teoretycznie wystarczy otworzyć `index.html` w przeglądarce. Ja osobiście wolę [serve](https://github.com/vercel/serve). Jest też deploy na GitHub Pages: https://ab9st8.github.io/wdai/proj1/.

Lokalnie, można rozszerzyć / ulepszyć działanie na dwa sposoby:

- Na stronie "about this" / faq.html widnieje forma do "kontaktu". Przycisk submit wykonuje POSTa na localhost:6969/notification --- w folderze server/ jest kod, który po wykonaniu `npm install && npm start` będzie prowizorycznie przyjmował te POSTy (przy pomocy kolejnej niewspomnianej zależności, expressa) i wyświetlał je w konsoli, dla dodania realizmu.
